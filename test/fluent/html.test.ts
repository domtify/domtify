...
describe('html', () => {
  ...
  it('获取 innerHTML', () => {
    const res = html()(dom('.demo-container'))
    expect(res.trim()).toBe('<div class="demo-box">Demonstration Box</div>')
  })

  it('设置html-字符串', () => {
    html('<p>Hello <strong>World</strong></p>')(dom('.demo-container'))
    const res = html()(dom('.demo-container'))
  ...
  it('设置 innerHTML-函数', () => {
    ...
    const result = html(fn)(dom('.demo-container'))
  ...
  it('当匹配不到元素时返回 undefined', () => {
    const res = html()(dom('.non-existent'))
