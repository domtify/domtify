...
import { parentsUntil } from '@/method/parentsUntil'
...
describe('parentsUntil', () => {
  ...
  it('获取所有父级元素（无参）', () => {
    const res = parentsUntil()(dom('li.item-a'))
  ...
  it('传递 selector，遇到停止元素（选择器）', () => {
    const res = parentsUntil('li.item-ii')(dom('li.item-a'))
  ...
  it('支持第二个参数：过滤器', () => {
    const res = parentsUntil('.level-1', '.yes')(dom('li.item-a'))
    expect(res.every(p => p.classList.contains('yes'))).toBe(true)
  })

  it('去重验证（重复节点不会多次出现）', () => {
    const res = parentsUntil('.level-1')(dom('li.item-a, li.item-b'))
