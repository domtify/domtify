...
### .append( content [, content ] ) <small style="font-size: 12px; color: var(--vp-c-green);">from @1.0</small>
...
### .append( function ) <small style="font-size: 12px; color: var(--vp-c-green);">from @1.0</small>
...
- **function**
  ...
  - **描述:** 返回 HTML 字符串、DOM 元素、文本节点或 domtify 对象，用于插入到匹配元素集合中每个元素的末尾。接收元素在集合中的索引位置和元素的旧 HTML 值作为参数。在函数内部，`this`引用集合中的当前元素。

该`.append()`方法将指定的内容插入为 domtify 集合中每个元素的最后一个子元素。
