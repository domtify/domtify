...
### .prevUntil( [selector ] [, filter ] ) <small style="font-size: 12px; color: var(--vp-c-green);">from @1.0</small>
...
### .prevUntil( [element ] [, filter ] ) <small style="font-size: 12px; color: var(--vp-c-green);">from @1.0</small>
...
与[.prevAll()](/reference/method-prevAll)方法效果相同,但是它支持在某个地方停止。
...
查找`#term-2`之前的兄弟元素直到`dt`元素结束，并过滤出携带`.a`类名的元素：
