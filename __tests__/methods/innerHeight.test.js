...
describe("innerHeight", () => {
  ...
  it("获取 window 高度", () => {
    ...
    expect(d(window).innerHeight()).toBe(800)

    Object.defineProperty(window, "innerHeight", {
      value: 600,
      configurable: true,
    })
    expect(d(window).innerHeight()).toBe(600)
  })

  it("获取 document 高度", () => {
    document.body.style.height = "1000px"
    document.documentElement.style.height = "980px"
    expect(d(document).innerHeight()).toBe(1000)
  })

  it("border-box 元素", () => {
    const result = d(".border-box").innerHeight()

    expectPixelEqual(result, 30.8)
  })

  it("content-box 元素", () => {
    const result = d(".content-box").innerHeight()
    expect(result).toBe(50)
  })

  it("设置高度", () => {
    d("div").innerHeight(100)
    expectPixelEqual(borderBoxEl.style.height, "119.2px")
    expectPixelEqual(contentBoxEl.style.height, "100px")
  })

  it("设置高度-数字字符串", () => {
    d("div").innerHeight("100.1")
