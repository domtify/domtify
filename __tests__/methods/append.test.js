...
describe("append", () => {
  ...
  it("数组", () => {
    const h2 = document.querySelector("h2")
    d(".inner").append([h2, "X", "<i>italic</i>"])
  ...
  it("支持函数返回字符串", () => {
    d(".inner").append((i, html) => `<b>idx-${i}</b>`)
  ...
  it("函数返回HTMLCollection集合", () => {
    d(".inner").append(() => document.getElementsByTagName("p"))
    expect(document.querySelectorAll(".inner p").length).toBe(2)
  })

  it("支持函数返回数组", () => {
    d(".inner").append(() => ["123", "<em>456</em>"])
    const inner = document.querySelector(".inner")
