...
describe("uniqueSort", () => {
  ...
  it("能正确去重", () => {
    const divs = query("div").concat(query(".dup")) // 共 9 个
    const res = uniqueSort()(query(divs))
    expect(res.length).toBe(6) // 最终只有 6 个
  })

  it("能过滤掉非 Element 节点", () => {
    const arr = [document.querySelector(".item-1"), 123, "abc", null]
    const res = uniqueSort()(query(arr))
  ...
  it("如果集合里全是重复元素，最后只保留一个", () => {
    ...
    const res = uniqueSort()(query([elItem, elItem, elItem]))
    expect(res.length).toBe(1)
    expect(res[0]).toBe(elItem)
  })

  it("空集合调用也能返回自身，不报错", () => {
    const res = uniqueSort()(query([]))
