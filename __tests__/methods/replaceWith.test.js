...
describe("replaceWith", () => {
  ...
  it("可以用 HTML 字符串替换元素", () => {
    replaceWith("<h2>New heading</h2>")(query("div.second"))
  ...
  it("可以用 Element 替换元素", () => {
    ...
    replaceWith(p)(query("div.second"))
    expect(document.querySelector(".inner.second")).toBeNull()
    expect(document.querySelector("p")).not.toBeNull()
  })

  it("可以用 TextNode 替换元素", () => {
    replaceWith(document.createTextNode("TextNode"))(query("div.second"))
    expect(document.querySelector(".inner.second")).toBeNull()
  ...
  it("可以用数组替换元素（包含 Element 和 HTML 字符串）", () => {
    const p = document.querySelector("p")
    replaceWith([p, "<h2>Array heading</h2>"])(query("div.second"))
    expect(document.querySelector(".inner.second")).toBeNull()
    expect(document.querySelector("h2").textContent).toBe("this is h2")
  })

  it("可以用集合对象替换元素", () => {
    replaceWith(query("p"))(query("div.second"))
    expect(document.querySelector(".inner.second")).toBeNull()
    expect(document.querySelector("p")).not.toBeNull()
  })
  ...
  it("函数形式可以返回字符串", () => {
    ...
    replaceWith(fn)(query("div.second"))
  ...
  it("非 Element 节点会被跳过（比如数字）", () => {
    replaceWith(() => "<b>ok</b>")(
      query([10, document.querySelector("div.second")]),
    )
    expect(document.querySelector("b").textContent).toBe("ok")
  })

  it("空集合调用时不会报错", () => {
    const res = replaceWith("<span>noop</span>")(query([]))
