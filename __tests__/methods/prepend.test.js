...
describe("prepend", () => {
  ...
  it("支持 HTML 字符串", () => {
    prepend("<span>Test</span>")(query(".inner"))
  ...
  it("元素数组", () => {
    const h2s = query("h2")
    prepend(h2s)(query(".inner"))
    expect(document.querySelectorAll("h2").length).toBe(2)
  })

  it("支持函数返回字符串", () => {
    prepend((i, html) => `<b>idx-${i}</b>`)(query(".inner"))
  ...
  it("函数返回HTMLCollection集合", () => {
    prepend(() => document.getElementsByTagName("p"))(query(".inner"))
    expect(document.querySelectorAll(".inner p").length).toBe(2)
  })

  it("支持函数返回数组", () => {
    prepend(() => ["123", "<em>456</em>"])(query(".inner"))
    const inner = document.querySelector(".inner")
