...
describe("closest", () => {
  ...
  it("支持原生 NodeList/HTMLCollection 作为 selector", () => {
    ...
    const result = d("li.item-a").closest(nodeList)
    expect(result.length).toBe(1)
    expect(result[0].classList.contains("item-a")).toBe(true)
  })

  it("多个元素共同寻找最近的 ul", () => {
    const result = d(".item").closest("ul")
    const tagNames = result.toArray().map((el) => el.tagName)
