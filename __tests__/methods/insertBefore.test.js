...
describe("insertBefore", () => {
  ...
  it("支持选择器", () => {
    insertBefore(".inner")(query("<p>Test</p>"))
  ...
  it("支持字符串(字符串实际上不会有任何反应,还是原来的dom结构)", () => {
    insertBefore("<div>Fake</div>")(query("<p>Invalid</p>"))
    expect(document.body.innerHTML).not.toContain("Invalid")
  })

  it("支持元素", () => {
    insertBefore(document.querySelector("h2"))(query("<p>Before h2</p>"))
    const container = document.querySelector(".container")
  ...
  it("支持数组", () => {
    ...
    insertBefore(targets)(query("<span>Multi</span>"))
  ...
  it("空集合: 不报错", () => {
    expect(() => insertBefore(".not-exist")(query("<p>xx</p>"))).not.toThrow()
  })

  it("返回元素数组", () => {
    const pEl = query("<p>Chain</p>")
